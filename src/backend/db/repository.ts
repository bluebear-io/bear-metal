...
export interface Repository {
  ...
  /** Per-worker status intervals across `[sinceMs, untilMs)` for the Gantt chart. */
  listWorkerTimeline(options: { sinceMs: number; untilMs: number }): Promise<WorkerTimelineResult>;
...
export function createRepository(handle: DbHandle): Repository {
  ...
  return {
    ...
    async listWorkerTimeline(options) {
      return computeWorkerTimeline(db, t, options);
    },
 ...
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function computeWorkerTimeline(db: any, t: any, options: { sinceMs: number; untilMs: number }): Promise<WorkerTimelineResult> {
  const { sinceMs, untilMs } = options;
  const workerRows: Worker[] = await db.select().from(t.workers).orderBy(asc(t.workers.name));
  ...
  type HistoryRow = { workerId: string; status: Worker["status"]; currentRunId: string | null; changedAt: Date };
  const inWindow: HistoryRow[] = await db.select().from(t.workerStatusHistory)
    .where(and(
      inArray(t.workerStatusHistory.workerId, workerIds),
      gte(t.workerStatusHistory.changedAt, since),
      lt(t.workerStatusHistory.changedAt, until),
    ))
    .orderBy(asc(t.workerStatusHistory.workerId), asc(t.workerStatusHistory.changedAt));
  ...
  const priorRows: HistoryRow[] = await db.select().from(t.workerStatusHistory)
    .where(and(
      inArray(t.workerStatusHistory.workerId, workerIds),
      lt(t.workerStatusHistory.changedAt, since),
    ))
    .orderBy(asc(t.workerStatusHistory.workerId), asc(t.workerStatusHistory.changedAt));
  const anchorByWorker = new Map<string, HistoryRow>();
  for (const r of priorRows) anchorByWorker.set(r.workerId, r); // last write wins → most recent prior

  const inWindowByWorker = new Map<string, HistoryRow[]>();
  for (const r of inWindow) {
