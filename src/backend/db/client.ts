...
const SCHEMA_SQL_SQLITE = `
  ...
  CREATE TABLE IF NOT EXISTS worker_status_history (
    id TEXT PRIMARY KEY NOT NULL, worker_id TEXT NOT NULL REFERENCES workers(id),
    status TEXT NOT NULL, current_run_id TEXT, changed_at INTEGER NOT NULL
  );
  CREATE INDEX IF NOT EXISTS worker_status_history_worker_id_changed_at_idx
    ON worker_status_history (worker_id, changed_at);
...
const SCHEMA_SQL_PG = `
  ...
  CREATE TABLE IF NOT EXISTS worker_status_history (
    id TEXT PRIMARY KEY NOT NULL, worker_id TEXT NOT NULL,
    status TEXT NOT NULL, current_run_id TEXT, changed_at TIMESTAMPTZ NOT NULL
  );
  CREATE INDEX IF NOT EXISTS worker_status_history_worker_id_changed_at_idx
    ON worker_status_history (worker_id, changed_at);
  CREATE TABLE IF NOT EXISTS runs (
