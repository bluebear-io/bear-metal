...
export interface DashboardClient {
  ...
  recordRunToolCall(p: RunToolCallPayload): Promise<void>;
}
...
export function createDashboardClient(options: DashboardClientOptions): DashboardClient {
  ...
  return {
    ...
    recordRunToolCall: (p) => send("PUT", `/api/runs/${encodeURIComponent(p.runId)}/tool-calls/${encodeURIComponent(p.id)}`, p),
  };
}
